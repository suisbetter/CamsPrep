...
|---|---|---|---|
| 1 | FAQ post 8129 has zero `<h1>` tags (Elementor desync, root cause 1.3) | Needs DB access or a human editing live in Elementor. **New finding (2026-08-07): an unpublished, corrupted draft (stray character, missing question number) currently sits in this post's Elementor editor — it is NOT live, but publishing anything from that editor session would push it live alongside any fix.** Do not open this post in Elementor and click Publish without first resolving that draft. | 🔍 Open, higher-risk than previously documented |
