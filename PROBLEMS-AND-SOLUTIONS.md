...
|---|---|---|---|
| 1 | FAQ post 8129 has zero `<h1>` tags (Elementor desync, root cause 1.3) | Needs DB access or a human editing live in Elementor — two automated attempts failed/corrupted and were discarded before publish. **Do not attempt more automated fixes on this post.** | 🔍 Open |
...
| 12 | Single-review (`ratingCount: 1`) AggregateRating on the free-test page reads as manipulated for a $2,000+ decision | The review is real, not fabricated — this is a marketing judgment call, not a data-integrity bug. Left as-is pending the user's decision. | ⚠️ Needs user decision |
...
| 11 | Unsubstantiated, conflicting experience-claim numbers ("2,000+ candidates" vs. "1,500+ professionals") | Not resolved — needs the user to confirm which figure (if either) is accurate. | ⚠️ Needs user decision |
...
| Missing-`<h1>` bug is broader than post 8129 — 2 more posts found with zero `<h1>` | Scope re-confirmed via a full 25-post sitemap sweep: exactly these 3 posts affected, not template-wide. Root cause (Elementor desync) unfixed — same "no automated attempts" rule applies. | 🔍 Open, scope contained |
...
| The Rank Math redirect table went completely empty at least once, silently reverting a previously-verified batch of 6-12 redirects | Recreated all redirects, turned on WP Ghost's Events Log (was off, so there was previously no audit trail for *any* admin action). One theory (automated Duplicator restore) ruled out — this site runs Duplicator Lite, which lacks the Pro-only scheduled-restore feature. Leading unconfirmed lead: WP Ghost's firewall log shows active DB-manipulation-tool probing (`searchreplacedb2.php`, `dump.sql`, a backdoor-shaped path) against the site in the same window, all "Prevented" — connection to the wipe not proven. | 🔍 Open — root cause still unconfirmed, but now traceable if it recurs |
...
| `/cams-test-preparation/` is hard-linked (still 404s) from a shared sidebar/footer widget on ~14+ other posts — distinct from the hub-page fix above | Not fixed — needs locating the shared widget/template and repointing it once. | ⚠️ Open |
